...
分野ごとに専用の仕組みを用意していて、組み立てる・見て確かめる・条件を変える・実験する、と関わり方が変わります。同じ画面の使い回しにならないようにしているところが、作っていて一番時間のかかるところです。`,
  ...
  },
  {
    slug: 'project-wlclab',
    title: 'WlcLab',
    thumbnail: `${base}images/works/project-wlclab/project-wlclab-thumb.png`,
    description: '無線 LAN コントローラ（Catalyst 9800）の管理画面を模した操作ドリル。CLI だけでなく、実機と同じ順序で GUI を操作しないと設定が入らないところまで再現しています。試作版を本番公開中。',
    detail: `WlcLab — 無線 LAN コントローラの操作ドリル（試作版・本番公開中）

無線の設定は、実務でも試験でも管理画面から行うことがほとんどです。ところが練習する手段がありません。そこで Catalyst 9800 の管理画面を模したエミュレーターを作り、ダイアログを開き、タブを切り替え、適用するところまで実機と同じ順序で操作する形にしました。

採点は、模範解答を流したときの状態との差分で行います。「壊してはいけないこと」を条件として書き並べるのは難しいためです。CCNA・WLCOR・WLSI のコース別に順序立てて練習でき、操作の手順を読み上げる音声ガイドも付けています。

Catalyst Center 風のコントローラ画面、Catalyst SD-WAN Manager 風の管理画面も試作していて、それぞれ別エンジンとして育てています。`,
    tech: ['React', 'TypeScript', 'Vite', 'TailwindCSS', 'Vitest'],
    productUrl: 'https://wlclab.sorapenguin.dev',
    githubUrl: null,
    sections: [
      {
        images: [
          `${base}images/works/project-wlclab/project-wlclab-1.png`,
          `${base}images/works/project-wlclab/project-wlclab-2.png`,
          `${base}images/works/project-wlclab/project-wlclab-3.png`,
          `${base}images/works/project-wlclab/project-wlclab-4.png`,
        ],
        caption: '無線 LAN コントローラの WebUI（WLANs・RF プロファイル・セキュリティ設定）を、実機と同じ画面構成・同じ操作順序で再現しています。RF プロファイルの調整、AP の管理、TACACS+ によるログイン認証の切り替え、SSID の新規作成まで、CLI だけでなく GUI 操作そのものを練習できます。',
      },
    ],
  },
  {
    ...
    title: 'GogoLab',
    thumbnail: `${base}images/works/project-gogolab/project-gogolab-thumb.png`,
    description: '情報処理技術者試験の午後問題（記述式・論文式）を、ブラウザで演習できるサイト。複数の試験区分をカバーし、書いた答案は採点用のプロンプトをコピーして手元の AI に評価してもらえます。',
    detail: `GogoLab — 記述式・論文式の演習サイト

午後試験は記述式・論文式のため、書いたあとに「これで何点なのか」が分からないまま終わりがちです。そこで、解答したあとに採点用のプロンプトを組み立ててコピーできる形にしました。手元の AI に貼れば、採点基準や評価の観点に沿った講評が返ってきます。

記述式はネットワークスペシャリストを中心に複数の試験区分の問題を、論文式は SA・ST・AU・PM・SM の区分を用意しています。論文式には唯一の正解がないため、参考答案は模範解答ではなく「こう書けばいい」の見本として、自分の経験で書き直す前提で置いています。答案には字数チェックや紋切り型表現の検出といったセルフチェック、120 分の練習タイマーも付けています。AI からの評価を貼り戻して記録しておける機能もあり、自分の答案がどう変わったかをあとから見返せます。`,
    ...
    sections: [
      {
        images: [
          `${base}images/works/project-gogolab/project-gogolab-1.png`,
          `${base}images/works/project-gogolab/project-gogolab-2.png`,
          `${base}images/works/project-gogolab/project-gogolab-3.png`,
          `${base}images/works/project-gogolab/project-gogolab-4.png`,
        ],
        caption: '長文シナリオを読みながら、ログや設定を自分で調べて記述式の設問に答えます。試験区分ごとに調査できるログの種類や設問の形式（選択・短答・記述）が変わり、提出後は模範解答と採点基準を見ながら振り返れます。',
      },
    ],
  },
  {
...
構成: Python / Django / Gunicorn / PostgreSQL / Redis。Docker + Traefik で VPS に配置し、Prometheus / Grafana で監視。GitHub Actions で CI。`,
  ...
  },
  {
    ...
    description: 'Cisco 機器の CLI をブラウザで練習できる学習サービス。700 問以上のシナリオに加え、検証・自動化・フィールド型（RLQ）という毛色の違う 4 つのラボがあります。React + TypeScript 製・VPS で本番公開中。',
    detail: `RouteLab — Cisco CLI 演習サービス（本番公開中）

自分がコマンド学習で詰まったのが作ったきっかけです。

ブラウザ上の疑似 CLI で機器を設定し、show コマンドで結果を確認します。当初は設定構築とトラブルシューティングの 2 系統だけでしたが、その後、設定済みの環境から不具合の原因を切り分ける「検証ラボ」、Ansible / Terraform / NETCONF / RESTCONF / Python で組む「自動化ラボ」を追加し、さらに CLI 入力を離れて図とポートを操作して解く「RouteLabQuest」というフィールド型モードも作りました。目的の違う 4 つのラボを、同じ採点の仕組みの上で動かしています。

演習の型も、選択式・入力式・写経・4 択・音声暗記・カードの 6 種類から選べます。手を動かせない時間は音声暗記モードで復習し、コマンド自体を覚え直したいときはシナリオから自動生成した暗記ドリルを使います。

構成: React / TypeScript / Vite / Zustand / TailwindCSS、Express + PostgreSQL（採点・認証）、姉妹サービスとの JWT SSO 連携。VPS 上に Docker + Traefik で配置し HTTPS 公開。`,
    tech: ['React', 'TypeScript', 'Vite', 'Zustand', 'TailwindCSS', 'React Router', 'Express', 'Node.js', 'PostgreSQL', 'JWT', 'Docker', 'Traefik', 'VPS'],
    ...
    sections: [
      {
        ...
        caption: 'タブレット / デスクトップでは3ペイン構成で表示されます。左ペインに MISSION（問題文・ネットワーク構成・必須採点条件）、中央ペインに CLI ターミナル、右ペインにコマンドボタンや演習モードの切り替えが並びます。設定ラボは選択式・入力式・写経・4択・音声暗記・カードの6種類の演習モードを切り替えられ、同じシナリオを違う角度から繰り返し練習できます。スマートフォンでも同じシナリオを操作できます。',
      },
      {
        images: [
        ...
        caption: '設定ラボ以外にも3つのラボがあります。検証ラボは設定済みの環境からACLやSTPなどの不具合を切り分ける練習、自動化ラボはAnsibleのプレイブックなどを組んで機器に反映する練習です。RouteLabQuest（RLQ）は、CLI入力を離れて、マップ上のラックを歩いて回り、現場ごとの不具合を直していくフィールド型のモードです。',
      },
    ],
  },
  {
...
構成: Kotlin / Jetpack Compose / Clean Architecture。バックエンドは Ktor。`,
  ...
  },
  {
    slug: 'project-i',
    title: '本番サービス運用基盤',
    thumbnail: `${base}images/works/project-i/project-i-cover.svg`,
...
構成: Ubuntu / Docker / Docker Compose / Traefik / PostgreSQL / Prometheus / Grafana / Cloudflare。問題コンテンツ生成用にローカル LLM のパイプラインを併設。`,
  ...
  },
  {
    ...
    thumbnail: `${base}images/works/project-g/project-g-cover.svg`,
...
構成: Go 標準ライブラリのみ、embed.FS でシングルバイナリ。Docker + Traefik で VPS に配置。`,
  ...
  },
  {
    ...
    title: 'WallLab',
    thumbnail: `${base}images/works/project-walllab/project-walllab-thumb.png`,
    description: 'Cisco ASA ファイアウォールの CLI を練習するドリル。RouteLab の Cisco IOS 版に対する ASA 版で、180 本のシナリオを収録しています。React + TypeScript 製・公開準備中。',
    detail: `WallLab — Cisco ASA CLI 演習サービス（開発中）

RouteLab と同じ「実際にコマンドを打つ」形式を、ファイアウォールに広げた作品です。ASA は IOS と別方言で、実機も用意しにくい領域です。

インターフェース設定、ACL と access-group、NAT、object-group、管理アクセスから、フェイルオーバーやマルチコンテキスト、リモートアクセス VPN、QoS まで扱っています。演習モードは選択式・入力式・写経・音声暗記・カードの 5 種類です。

構成: React / TypeScript / Vite / TailwindCSS / Zustand、Express（採点 API）。RouteLab と同様、公開はこれから行う予定です。`,
    tech: ['React', 'TypeScript', 'Vite', 'TailwindCSS', 'Zustand', 'Express', 'Vitest'],
    productUrl: null,
    githubUrl: null,
    inProgress: true,
    sections: [
      {
        images: [
          `${base}images/works/project-walllab/project-walllab-1.png`,
          `${base}images/works/project-walllab/project-walllab-2.png`,
          `${base}images/works/project-walllab/project-walllab-3.png`,
          `${base}images/works/project-walllab/project-walllab-4.png`,
        ],
        caption: 'ACL のロギング設定から AnyConnect（SSL-VPN）の立ち上げまで、Cisco ASA でよく使う設定を CLI で組み立てます。3 ペイン構成で MISSION・CLI ターミナル・コマンドボタンを表示し、スマートフォンでも同じシナリオを操作できます。',
      },
    ],
  },
  {
    slug: 'project-cdlab',
    title: 'CLIDrillLab',
    thumbnail: `${base}images/works/project-cdlab/project-cdlab-thumb.png`,
    description: 'Linux（LPIC）を中心に、Docker・Kubernetes も交えたタイピング式の CLI ドリル。React + TypeScript 製・本番公開中。',
    detail: `CLIDrillLab — Linux / コンテナの CLI ドリル（本番公開中）

LPIC の上位試験を勉強していて、追加でやれる教材が少なかったのが作ったきっかけです。RouteLab の Cisco 版に対する Linux 版にあたります。

「症状の確認 → 調査 → 対処 → 復旧確認」の順に、数ステップのコマンドを続けて打ちます。LPIC の複数レベルを中心に、Docker・Kubernetes の典型的な詰まり方も扱っています。演習モードは RouteLab と同じ、選択式・入力式・写経・4 択・音声暗記・カードの 6 種類です。

シナリオとは別に、コマンド・サブコマンド・オプションを一問一答で覚え直せる暗記モード（/memo）もあります。姉妹サービスの Tech-Lab とアカウントを連携しています。

構成: React / TypeScript / Vite / TailwindCSS。`,
    tech: ['React', 'TypeScript', 'Vite', 'TailwindCSS'],
    productUrl: 'https://cli-drill-lab.sorapenguin.dev',
    githubUrl: null,
    sections: [
      {
        images: [
          `${base}images/works/project-cdlab/project-cdlab-1.png`,
          `${base}images/works/project-cdlab/project-cdlab-2.png`,
          `${base}images/works/project-cdlab/project-cdlab-3.png`,
          `${base}images/works/project-cdlab/project-cdlab-4.png`,
        ],
        caption: 'LPIC のコマンド操作を選択式・4 択などのモードで練習できるほか、コマンドだけを繰り返し覚え直せる暗記モード（/memo）も別に用意しています。Docker のようなコンテナ操作も同じ形式で扱っていて、スマートフォンでも操作できます。',
      },
    ],
  },
  {
