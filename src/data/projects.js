...
オフライン進捗はサーバー時刻を基準に計算してデバイス時計の改ざんを防止し、長期放置時にはアシスタントギフトを付与する設計にしています。JWT トークンは端末内で暗号化保存し、Ktor バックエンドとのバックグラウンド同期でクラウドセーブを実現。クラウドセーブはバックグラウンドで定期同期し、起動時にローカルと API のタイムスタンプを比較して新しい方を採用する競合解決設計です。`,
    ...
    sections: [
      ...
      },
      {
        ...
        caption: '工房の実績タブでは錬金士ランク（XP ベース・10 段階）と全 22 種のアチーブメント達成状況（全元素発見 69/75 など）を確認できます。機器タブでは火・水・土・風の採集器を Gold で強化し、Lv.15（MAX）で各素材を最速自動収集します。錬成炉では T3 以上の素材を最大 3 スロット並列バッチ生産でき、放置中にまとめて製造が進みます。設定画面では 72 時間有効の引継ぎコードを発行して別端末へのデータ移行を安全に行えます。',
      },
    ],
  },
  {
